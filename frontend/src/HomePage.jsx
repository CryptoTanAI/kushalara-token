...
// NEW Cryptocurrency Payment Handlers 
const handleETHPayment = async (amount) => {
    ...
    try {
        ...
        if (typeof window.ethereum !== 'undefined') {
            ...
            const transactionParameters = {
                ...
                from: accounts[0],
                value: (amount * Math.pow(10, 18)).toString(16), // Convert to Wei in hex
            ...
            throw new Error('MetaMask not detected. Please install MetaMask to pay with ETH.');
        }
...
const handleBTCPayment = async (amount) => {
    console.log('🟡 BTC Payment Handler - Starting...');
    alert('Bitcoin payment: Please send ' + amount + ' BTC to address: ' + walletAddresses.BTC);
};
...
const handleSOLPayment = async (amount) => {
    ...
    try {
        ...
        if (window.solana && window.solana.isPhantom) {
            ...
            alert('Solana payment integration coming soon. Please use manual transfer for now.');
            
        } else {
            throw new Error('Phantom wallet not detected. Please install Phantom to pay with SOL.');
        }
...
const handleUSDCPayment = async (amount) => {
    console.log('🔵 USDC Payment Handler - Starting...');
    alert('USDC payment: Please send ' + amount + ' USDC to address: ' + walletAddresses.USDC);
};

const handleUSDTPayment = async (amount) => {
    console.log('🟢 USDT Payment Handler - Starting...');
    alert('USDT payment: Please send ' + amount + ' USDT to address: ' + walletAddresses.USDT);
};
